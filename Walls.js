class Walls extends PIXI.Container {
  constructor() {
    ...
    this.VIEWPORT_NUM_SLICES =
      Math.ceil(this.VIEWPORT_WIDTH / WallSlice.WIDTH) + 1;
  ...
  checkViewportXBounds(viewportX) {
    var maxViewportX =
      (this.slices.length - this.VIEWPORT_NUM_SLICES) * WallSlice.WIDTH;
    if (viewportX < 0) {
      viewportX = 0;
  ...
  addNewSlices() {
    ...
    for (
      var i = this.viewportSliceX, sliceIndex = 0;
